...
local windmill_mt = { __index = windmill }

function windmill.new( name, description, image )
	local newWindmill = {
	name = windmill,
	...
	return setmetatable( newWindmill, windmill_mt )
end

-- Windmill Accumulation Function
function windmill:enerygyAccumulationRate()
...
function windmill:cost()
	user.energy = user.energy - 100 
	user.energy.text = user.energy
	return user.energy 
end

function windmill:requiredLevel()
	user.level = 10  
end
