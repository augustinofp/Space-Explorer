local blackhole = {} -- table representing class planet

blackhole.__index = blackhole --failed table lookups should fall back to class table to get methods 
--type, toxicity, age, colours, level, 	
function blackhole.new(image, gal_ID, ssimage)
...
function blackhole.set_image(self, image)
	self.image = image
end

function blackhole.get_image(self)
	return self.image
end

function blackhole.set_galID(self, gal_ID)
	self.sys_ID = sys_ID
end

function blackhole.get_galID(self)
	return self.gal_ID
end
